...
const calculateCartTotal = (cartItems = {}, products = []) => {
  const priceMap = new Map(
    products.map((item) => [item._id.toString(), item.price])
  );

  return Object.keys(cartItems).reduce((total, productId) => {
    const qty = cartItems[productId] || 0;
    const price = priceMap.get(productId.toString()) || 0;
...
const getCartByUserId = async (req, res) => {
  try {
    ...
    const products =
      productIds.length > 0
        ? await foodModel.find({ _id: { $in: productIds } })
        : [];
